...
import (
	dpfm_api_caller "data-platform-api-product-type-reads-rmq-kube/DPFM_API_Caller"
	...
	"data-platform-api-product-type-reads-rmq-kube/config"
	"encoding/json"
	"fmt"
	"time"
...
func getAccepter(input *dpfm_api_input_reader.SDC) []string {
	...
	if accepter[0] == "All" {
		accepter = []string{
			...
			"ProductTypeText", "ProductTypeTexts",
		}
	}
	return accepter
}
